...
trait ResumableRecordDataFeeder extends RecordDataFeeder {
  ...
  def toContextData(data: TaskData): ContextData = data.fields + ("token" -> data.token)

  def fromContextData(data: ContextData): TaskData = TaskData(data("token").toString.toLong, fields = data - "token")

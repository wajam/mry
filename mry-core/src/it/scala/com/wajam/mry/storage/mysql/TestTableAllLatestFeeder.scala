...
class TestTableAllLatestFeeder extends FunSuite with MysqlStorageFixture {
  ...
  test("feeder should feed in loop a table and in token+key order") {
    withFixture { f =>
      ...
      }, commit = true, onTimestamp = NrvTimestamp(0))


      val feeder = new TableAllLatestFeeder("test", f.mysqlStorage, table1, TokenRange.All) with TableContinuousFeeder
  ...
  test("init with context") {
    withFixture { f =>
      ...
      for (((token, key), i) <- keys.zipWithIndex) {
        println(s"$token, $key, $i")
        f.exec(t => {
          val t1 = t.from("mysql").from("table1")
          t1.set(key, Map(key -> key))
        }, commit = true, onTimestamp = NrvTimestamp(i))
      }
      ...
      feederContext.data += (Timestamp -> NrvTimestamp(5))
  ...
  test("save and resume context with multi level table") {
    withFixture { f =>
      ...
      // Load some records
      val feeder1 = new TableAllLatestFeeder("test", f.mysqlStorage, table1_1, TokenRange.All) with TableContinuousFeeder
      ...
      val feeder2 = new TableAllLatestFeeder("test", f.mysqlStorage, table1_1, TokenRange.All) with TableContinuousFeeder
  ...
  test("feeder should iterate a table and in token+key order for ranges") {
    withFixture { f =>
      ...
      val feeder = new TableAllLatestFeeder("test", f.mysqlStorage, table1, ranges, loadLimit = 3) with TableContinuousFeeder
      val records = feeder.take(100).flatten.toList
  ...
  test("should not stop prematurely when iterating over many consecutive deleted records") {
    withFixture { f =>
      ...
      def delete(record: Record) {
        val transaction = f.mysqlStorage.createStorageTransaction
        try {
      ...
      val feeder = new TableAllLatestFeeder("test", f.mysqlStorage, table1_1_1, TokenRange.All, loadLimit = 3) with TableContinuousFeeder

...
class TestTableTombstoneFeeder extends FunSuite with MysqlStorageFixture {

  test("should extract token from tombstone record") {
    withFixture { f =>
      val table = table1
      val ranges = List(TokenRange.All)
      val feeder = new TableTombstoneFeeder("test", f.mysqlStorage, table, ranges, 0) with TableContinuousFeeder
  ...
  test("should convert record to data and data to record") {
    withFixture { f =>
      ...
      val feeder = new TableTombstoneFeeder("test", f.mysqlStorage, table1_1, Seq(TokenRange.All), 0) with TableContinuousFeeder
  ...
  test("should load tombstone records") {
    withFixture { f =>
      ...
      f.mysqlStorage.setCurrentConsistentTimestamp((range) => currentConsistentTimestamp)

      val feeder = new TableTombstoneFeeder("test", f.mysqlStorage, table1_1, Seq(TokenRange.All),
      ...
      val limitedFeeder = new TableTombstoneFeeder("test", f.mysqlStorage, table1_1, Seq(TokenRange.All),
